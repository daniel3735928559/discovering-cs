...
app.controller("PySimController", ['$scope','$timeout',function($scope, $timeout){
    $scope.do_nothing = function(a){console.log();}
	...
	"array":function(args){
	    var val = [];
	    $scope.debug_log("building array",args);
	    for(var i = 0; i < args.length; i++){
		$scope.debug_log("pushing",args[i]);
		val.push(args[i]);
	    }
	    $scope.debug_log("built array",val);
	    return val;
	}
    };
    $scope.builtins = {
	...
	"lb":[
		...
		"run":function(args){
		    $scope.lightboard_on = true;
		    $scope.$apply();
		    $scope.debug_log("SETTING",(args[2].toString())[0]);
		    var b = $scope.get_button(args[0], args[1]);
	...
	"googlemaps":[
		...
		"run":function(args){
		    ...
		    $scope.debug_log("DIRDIR",args[0]," to ", args[1]);
			...
			if (status == google.maps.DirectionsStatus.OK) {
			    $scope.debug_log("GOTMAP",response)
			    var arr = [];
				...
				for(var j = 0; j < response.routes[0].legs[i].steps.length; j++){
				    ...
				    $scope.debug_log("IIII",instruction);
	...
	$scope.cm_setup();
	$scope.debug_log($scope.line_types);
    }
	...
	if($scope.click_callback != null){
	    ...
	    $scope.debug_log("REM",$scope.remaining_steps);
	...
	$scope.variables = {};
	$scope.functions = {};
	$scope.debug_log("BUILTINS",$scope.builtins);
	...
	$scope.debug_log("Getting",x);
	$scope.debug_log("ASDASD",$scope.get_line_num());
	...
	$scope.debug_log("Getting array ",x,"from",JSON.stringify($scope.variables),$scope.variables[x]);
	$scope.debug_log("ASDASD",$scope.get_line_num());
	if(x in $scope.variables){
	    var arr = $scope.variables[x];
	...
	$scope.debug_log("TS",JSON.stringify(to_search.state));
	...
	$scope.debug_log("FOUND CALLS",JSON.stringify(calls.state));
	return calls;
    }
    $scope.get_call = function(call_path,ast){
	$scope.debug_log("CP",JSON.stringify(call_path));
	var c = ast;
	for(var i = 0; i < call_path.length-1; i++){
	    $scope.debug_log("CC",JSON.stringify(c));
	    c = c[call_path[i]];
	}
	$scope.debug_log("FC",c);
	return c;
    }
	...
	$scope.debug_log("RETURNING TO",prev_state.line,prev_state.block.parent_line,JSON.stringify(prev_state.call_path),JSON.stringify(prev_state.scope));
	...
	$scope.debug_log(prev_state.ast);
	$scope.called_a_function = false;
	...
	$scope.debug_log("CALLS",calls.state);
	if(!(calls.is_empty())){
		...
		$scope.debug_log("FNFNFN",state.fn);
		$scope.debug_log(JSON.stringify(state.scope));
		$scope.call_stack.push(state);
	...
	if(ast[0] == 'val'){
	    $scope.debug_log("val",ast[1]);
	    if($scope.is_valid_string(ast[1])){
		ast[1] = $scope.handle_string_escapes(ast[1]);
	    }
	    return ast[1];
	}
	if(ast[0] == 'name'){
	...
	if(!($scope.is_valid_number(answer) || $scope.is_valid_string(answer) || $scope.is_valid_array(answer) || answer == true || answer == false)){
	    $scope.debug_log("invlid");
	    $scope.raise_error("Expression has no valid value: " + expr);
	    return;
	}
	if($scope.is_valid_string(answer)){
	    $scope.debug_log("escaping",answer);
	    answer = $scope.handle_string_escapes(answer);
	}
	else $scope.debug_log("ANS",answer);
	$scope.debug_log("THE ANSWER",answer);
	return answer;
    }
    $scope.context = [];
	...
	"if":{"regex":/^if\(((?:[a-zA-Z0-9_\+\-\.\*\/%()[\],=!>< ]+|"(?:[^"\\]|\\.)*")*)\): *$/,"execute":function(data){
	    $scope.debug_log(data);
	    var test = $scope.parse_expression(data[1]);
	    $scope.debug_log(test);
	    return test;
	}},
	...
	"return":{"regex":/^return *([a-zA-Z0-9_\+\-\.\*\/%()[\]," ]*)$/,"execute":function(data){
	    $scope.debug_log("D1",JSON.stringify(data[1]));
	...
	this.step = function(){
	    ...
	    $scope.debug_log(inst.type,inst.text);
	    $scope.updated = [];
	    if(inst.type == 'if'){
		var val = inst.run();
		if($scope.called_a_function){
		    $scope.called_a_function = false;
		    $scope.debug_log("if postponed");
		    return;
		}
		if(val){
		...
		if($scope.called_a_function){
		    $scope.called_a_function = false;
		    $scope.debug_log("while postponed");
		    return;
		}
		...
		if(self.line == 0 || self.lines[self.line - 1].type != 'if'){
		    ...
		    $scope.debug_log("if was taken");
		    self.advance();
		}
		else{
		    $scope.debug_log("if not taken--do else");
		    $scope.current_block = inst.block;
		    $scope.update_status();
		}
	    }
	...
	this.advance = function(){
	    $scope.debug_log("ADVANCE");
	    if($scope.error_happened){
		$scope.debug_log("uh oh");
		$scope.reset();
		$scope.status = $scope.error_message;
		return;
	    }
	    if($scope.called_a_function){
		$scope.debug_log("CAFF");
		$scope.called_a_function = false;
		return;
	    }
	    if(self.parent_line) $scope.debug_log(self.parent_line.text);
	    else $scope.debug_log("no parent");
	    self.line++;
	    $scope.debug_log(self.line, self.lines.length, self.lines[0]);
		...
		if(!self.parent_line){
		    ...
		    if($scope.called_a_function){
			$scope.called_a_function = false;
			$scope.debug_log("repeat postponed");
			return;
		    }
		    if(val){
			$scope.debug_log("repeating");
	...
	for(var l = 0; l < lines.length; l++){
	    ...
	    if(inst.error){
		$scope.debug_log("Got bad line");
		$scope.status = "Error at line " + (l+1) + ": " + inst.error;
		$scope.set_error_line(l);
		return;
	    }
	    inst.line_num = l+1;
	    $scope.debug_log(inst.text, inst.indent_level, current_level)
	...
	for(var i = 0; i < b.lines.length; i++){
	    var inst = b.lines[i];
	    $scope.debug_log(l,b.line,inst.text);
	    if(inst.block != null){
	...
	$scope.status = $scope.error_message;
	$scope.debug_log("ERRORERRORERROR",s);
    }
