...
app.controller("AvrSimController", function($scope){
    ...
    $scope.uarch_cycle = 0;
    $scope.uarch_inst_cycle = 0;
    $scope.uarch_trace = "statemachine-blank";
    $scope.sm_image = "/lib/uarch/pngs/statemachine-blank.png";
    $scope.uarch_image = "/lib/uarch/pngs/uarch-blank.png";
    $scope.uarch_regs = {
	"INST":0,
	"VAL":0,
	"VAL1":0,
	...
	"00100":{
		...
		return {"controls":$scope.set_controls({"OFF_we":1,"OFF_sel":0}),
			"uarch":$scope.set_uarch("OFF",$scope.truncate($scope.extract_bits(inst.encoding,11,0),12,true))};
	    },
	    "next":function(inst){
		return {
		    "rjmp":"01110"
	...
	"00110":{
	    "name":"VAL = RF[REG]",
	    "exec":function(inst){
		return {"controls":$scope.set_controls({"VAL_we":1,"VAL_sel":2,"RF_sel":0}),
	...
	"00111":{
	    "name":"VAL1 = RF[REG]",
	    "exec":function(inst){
		return {"controls":$scope.set_controls({"VAL1_we":1,"RF_sel":0}),
	...
	"01000":{
	    "name":"VAL2 = RF[REG2]",
	    "exec":function(inst){
		return {"controls":$scope.set_controls({"VAL2_we":1,"VAL2_sel":0,"RF_sel":1}),
	...
	"01011":{
	    "name":"VAL = VAL1 + VAL2",
	    "exec":function(inst){
		return {"controls":$scope.set_controls({"VAL_we":1,"VAL_sel":1,"A_sel":1,"B_sel":0,"ALU_op":0}),
	...
	"01100":{
	    "name":"VAL = VAL1 - VAL2",
	    "exec":function(inst){
		return {"controls":$scope.set_controls({"VAL_we":1,"VAL_sel":1,"A_sel":0,"B_sel":0,"ALU_op":1}),
			"uarch":$scope.set_uarch("VAL",$scope.truncate($scope.uarch_regs["VAL1"]-$scope.uarch_regs["VAL2"],8,false))};
	    },
	    "next":function(inst){
		return "01101";
	...
	"01110":{
	    "name":"VAL = OFF+PC",
	    "exec":function(inst){
		return {"controls":$scope.set_controls({"VAL_we":1,"VAL_sel":1,"A_sel":0,"B_sel":1,"ALU_op":1}),
	...
	"10010":{
		...
		return {"controls":$scope.set_controls({"RF_we":1,"RF_sel":0}),
			"uarch":["None"],
    ...
    $scope.display_pm_start = 0;
    $scope.display_ram_start = 0;
    $scope.display_pm_length = 8;
    $scope.display_ram_length = 8;
	...
	var trace = "Cycle: " + ($scope.uarch_inst_cycle+1) + "\n";
	...
	$scope.uarch_image = "/lib/uarch/pngs/uarch-"+i.mnemonic+"."+$scope.uarch_inst_cycle+".png";
	$scope.sm_image = "/lib/uarch/pngs/statemachine-"+i.mnemonic+"."+$scope.uarch_inst_cycle+".png";
	...
	for(var k = 0; k < $scope.steps.count; k++){
	    ...
	    $scope.uarch_inst_cycle = 0;
	    $scope.uarch_image = "/lib/uarch/pngs/uarch-"+inst.mnemonic+".all.png";
	    $scope.sm_image = "/lib/uarch/pngs/statemachine-"+inst.mnemonic+".all.png";
	    i.run();
	    if($scope.PC < $scope.display_pm_start || $scope.PC >= $scope.display_pm_start + $scope.display_pm_length){
		$scope.display_pm_start = Math.max(0, $scope.PC - $scope.display_ram_length/2);
	    }
